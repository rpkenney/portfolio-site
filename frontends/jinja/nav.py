"""Relative ``href``s for static HTML (``site.css``, ``js/carousel.js`` by depth)."""
...
def asset_hrefs(depth: int) -> tuple[str, str, str, str]:
    """CSS path, content-frame script, carousel script, Langton backdrop script."""

    prefix = "../" * depth
    return (
        f"{prefix}site.css",
        f"{prefix}js/content-frame.js",
        f"{prefix}js/carousel.js",
        f"{prefix}js/langton-ant.js",
    )
