(function () {
  ...
  /** Open height animation; mirror ``carouselOpenHeightMs`` in ``frontends/jinja/ui_config.py`` / CSS ``0.35s``. */
  const OPEN_HEIGHT_MS =
    typeof CFG.carouselOpenHeightMs === "number" ? CFG.carouselOpenHeightMs : 350;
  const VIEWPORT_BOTTOM_MARGIN_PX =
    typeof CFG.viewportBottomMarginPx === "number" ? CFG.viewportBottomMarginPx : 20;
  const HEIGHT_TRANSITION_END_FALLBACK_MS =
    typeof CFG.heightTransitionEndFallbackMs === "number"
      ? CFG.heightTransitionEndFallbackMs
      : 450;
  const CAROUSEL_CLOSE_TRANSITION_END_FALLBACK_MS =
    typeof CFG.carouselCloseTransitionEndFallbackMs === "number"
      ? CFG.carouselCloseTransitionEndFallbackMs
      : 600;
  ...
  /** Mobile modal: escape transformed ``.content-frame-stage`` so ``position: fixed`` hits the viewport. */
  let modalPortal = null;

  function restoreModalPortal() {
    if (!modalPortal) return;
    const { el, parent, next } = modalPortal;
    el.classList.remove("resume-section-web--modal-portal");
    if (next) parent.insertBefore(el, next);
    else parent.appendChild(el);
    modalPortal = null;
  }

  function syncModalPortal(modalOpen) {
    restoreModalPortal();
    if (!modalOpen || !expandedSection) return;
    const web = expandedSection.querySelector(".resume-section-web");
    if (!web || web.hidden) return;
    modalPortal = {
      el: web,
      parent: web.parentNode,
      next: web.nextSibling,
      section: expandedSection,
    };
    web.classList.add("resume-section-web--modal-portal");
    document.body.appendChild(web);
  }

  /** Carousel panel for a section (may be portaled to ``body`` on mobile). */
  function webPanelForSection(section) {
    if (!section) return null;
    if (modalPortal && modalPortal.section === section) return modalPortal.el;
    return section.querySelector(".resume-section-web");
  }
  ...
  function setExpandState(trigger, open) {
    if (!trigger) return;
    trigger.setAttribute("aria-expanded", open ? "true" : "false");
    const title = trigger.closest("section.resume-section")?.querySelector("h2")?.textContent?.trim();
    if (title) {
  ...
  function afterHeightTransition(
    wrapper,
    onDone,
    fallbackMs = HEIGHT_TRANSITION_END_FALLBACK_MS
  ) {
    let finished = false;
    const done = () => {
      if (finished) return;
  ...
  function focusExpandedCarousel(section) {
    const carousel = webPanelForSection(section);
    if (!carousel) return;
  ...
  function transitionTo(nextSection, options) {
    ...
    if (modalModeEnabled()) {
      if (from) {
        const carousel = webPanelForSection(from);
      ...
      else if (from && restoreFocusOnClose) {
        from.querySelector("[data-resume-carousel-expand]")?.focus({ preventScroll: true });
      }
      return;
    }
  ...
  function animateClose(section, restoreFocus, onDone) {
    ...
    afterHeightTransition(
      ...
      CAROUSEL_CLOSE_TRANSITION_END_FALLBACK_MS
    );
  ...
  function wireExpandTrigger(trigger) {
    const toggle = () => {
      const id = trigger.getAttribute("aria-controls");
      const carousel = id && document.getElementById(id);
      const section = trigger.closest("section.resume-section");
  ...
  document.querySelectorAll("[data-resume-carousel-expand]").forEach(wireExpandTrigger);
  ...
  document.addEventListener("keydown", (e) => {
    ...
    const activeCarousel = document.querySelector(
      ".resume-section-web--modal-portal:not([hidden]), .resume-section--web-active [data-resume-carousel]:not([hidden])"
    );
